...
sound_driver_t *get_ac97_driver(pci_device_header_t header, uint16_t bus,
                                uint16_t device, uint16_t function);
void ac97_pci_found(pci_device_header_t header, uint16_t bus, uint16_t device,
                    uint16_t function);
...
AC97Driver *NEW_AC97Driver(pci_device_header_t header, uint16_t bus,
                           uint16_t device, uint16_t function);
...
uint64_t AC97_writeBuffer(AC97Driver *driver, uint64_t offset, uint8_t *buffer,
                          uint64_t count);
...
// void write_word(uint64_t address, pci_bar_t type, uint16_t field, uint16_t
// value)
// {
...
static inline void write_mixer(AC97Driver *driver, MixerRegisters reg,
                               uint16_t val) {
  write_word((uint64_t)driver->PCIBaseAddress, driver->m_mixer_type, reg,
             val); // outw(m_mixer_address + reg, val);
  io_wait(10);
}
